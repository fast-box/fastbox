...
// node into the fast sync schedule.
func (this *fastSync) deliverHeaders(id string, headers []*types.Header) (err error) {
...
func (this *fastSync) deliverReceipts(id string, receipts [][]*types.Receipt) (err error) {
	return this.deliver(id, this.receiptCh, &receiptPack{id, receipts}, receiptInMeter, receiptDropMeter)
}

// DeliverNodeData injects a new batch of node state data received from a remote node.
func (this *fastSync) deliverNodeData(id string, data [][]byte) (err error) {
...
func (this *fastSync) registerPeer(id string, version uint, peer Peer) error {
...
func (this *fastSync) registerLightPeer(id string, version uint, peer LightPeer) error {
...
// the sch.
func (this *fastSync) unregisterPeer(id string) error {
...
func (this *fastSync) syncWithPeer(id string, p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	// If the point is below the origin, move origin back to ensure state fast sync
	if pivot < origin {
		if pivot > 0 {
			origin = pivot - 1
		} else {
	...
	if err != nil && this.fsPivotLock != nil {
		// If sync failed in the critical section, bump the fail counter.
		atomic.AddUint32(&this.syncer.fsPivotFails, 1)
	}
	return err
}
...
func (this *fastSync) spawnSync(fetchers []func() error) error {
...
func (this *fastSync) fetchHeight(p *peerConnection) (*types.Header, error) {
...
func (this *fastSync) findAncestor(p *peerConnection, height uint64) (uint64, error) {
...
// fetchBodies iteratively fast syncs the scheduled block bodies, taking any
...
// fetchReceipts iteratively fast syncs the scheduled block receipts, taking any
// available peers, reserving a chunk of receipts for each, waiting for delivery
// and also periodically checking for timeouts.
func (this *fastSync) fetchReceipts(from uint64) error {
	log.Debug("fast syncing transaction receipts", "origin", from)
	...
	log.Debug("Transaction receipt fast sync terminated", "err", err)
	return err
}

// fetchParts iteratively fast syncs scheduled block parts, taking any available
// peers, reserving a chunk of fetch requests for each, waiting for delivery and
// also periodically checking for timeouts.
//
// As the scheduling/timeout logic mostly is the same for all fast synced data
// types, this method is used by each for data gathering and is instrumented with
// various callbacks to handle the slight differences between processing them.
//
// The instrumentation parameters:
//  - errCancel:   error type to return if the fetch operation is cancelled (mostly makes logging nicer)
//  - deliveryCh:  channel from which to retrieve fast synced data packets (merged from all concurrent peers)
//  - deliver:     processing callback to deliver data packets into type specific fast sync schs (usually within `sch`)
//  - wakeCh:      notification channel for waking the fetcher when new tasks are available (or sync completed)
//  - expire:      task callback method to abort requests that took too long and return the faulty peers (traffic shaping)
//  - pending:     task callback for the number of requests still needing fast sync (detect completion/non-completability)
//  - inFlight:    task callback for the number of in-progress requests (wait for all active fast syncs to finish)
//  - throttle:    task callback to check if the processing sch is full and activate throttling (bound memory use)
//  - reserve:     task callback to reserve new fast sync tasks to a particular peer (also signals partial completions)
//  - fetchHook:   tester callback to notify of new tasks being initiated (allows testing the scheduling logic)
//  - fetch:       network callback to actually send a particular fast sync request to a physical remote peer
//  - cancel:      task callback to abort an in-flight fast sync request and allow rescheduling it (in case of lost peer)
//  - capacity:    network callback to retrieve the estimated type-specific bandwidth capacity of a peer (traffic shaping)
//  - idle:        network callback to retrieve the currently (type specific) idle peers that can be assigned tasks
//  - setIdle:     network callback to set a peer back to idle and update its estimated capacity (traffic shaping)
//  - kind:        textual label of the type being fast synced to display in log mesages
		...
		case <-update:
			...
			for pid, fails := range expire() {
				if peer := this.syncer.peers.Peer(pid); peer != nil {
					...
					//
					// The reason the minimum threshold is 2 is because the fast syncer tries to estimate the bandwidth
			...
			// Send a fast sync request to all idle peers, until throttled
			...
			for _, peer := range idles {
				...
				if err := fetch(peer, request); err != nil {
					...
					// case, the internal state of the fast syncer and the sch is very wrong so
...
// keeps processing and scheduling them into the header chain and fast syncer's
	...
	for {
		...
		case headers := <-this.headerProcCh:
			...
			if len(headers) == 0 {
				...
				// schd for processing when the header fast sync completes. However, as long as the
...
// qosReduceConfidence is meant to be called when a new peer joins the fast syncer's
// peer set, needing to reduce the confidence we have in out QoS estimates.
func (this *fastSync) qosReduceConfidence() {
	...
	log.Debug("Relaxed fast syncer QoS values", "rtt", rtt, "confidence", float64(conf)/1000000.0, "ttl", this.syncer.requestTTL())
}

// requestRTT returns the current target round trip time for a fast sync request
// to complete in.
//
// Note, the returned RTT is .9 of the actually estimated RTT. The reason is that
// the fast syncer tries to adapt queries to the RTT, so multiple RTT values can
// be adapted to, but smaller ones are preffered (stabler fast sync stream).
func (this *fastSync) requestRTT() time.Duration {
	return time.Duration(atomic.LoadUint64(&this.syncer.rttEstimate)) * 9 / 10
}

// syncState starts fast syncing state with the given root hash.
func (this *fastSync) syncState(root common.Hash) *stateSync {
	s := newStateSync(this.syncer, root)
	select {
	case this.syncer.stateSyncStart <- s:
	case <-this.syncer.quitCh:
