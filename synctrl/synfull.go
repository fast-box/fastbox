...
// node into the sync schedule.
func (this *fullSync) deliverHeaders(id string, headers []*types.Header) (err error) {
...
func (this *fullSync) deliverReceipts(id string, receipts [][]*types.Receipt) (err error) {
...
func (this *fullSync) registerLightPeer(id string, version uint, peer LightPeer) error {
...
// the sch.
func (this *fullSync) unregisterPeer(id string) error {
...
func (this *fullSync) spawnSync(fetchers []func() error) error {
...
func (this *fullSync) fetchHeight(p *peerConnection) (*types.Header, error) {
...
func (this *fullSync) fetchHeaders(p *peerConnection, from uint64) error {
	p.log.Debug("Directing header syncs", "origin", from)
	defer p.log.Debug("Header sync terminated")
...
// fetchBodies iteratively syncs the scheduled block bodies, taking any
...
// fetchReceipts iteratively syncs the scheduled block receipts, taking any
...
// fetchParts iteratively syncs scheduled block parts, taking any available
// peers, reserving a chunk of fetch requests for each, waiting for delivery and
// also periodically checking for timeouts.
//
// As the scheduling/timeout logic mostly is the same for all synced data
// types, this method is used by each for data gathering and is instrumented with
// various callbacks to handle the slight differences between processing them.
//
// The instrumentation parameters:
//  - errCancel:   error type to return if the fetch operation is cancelled (mostly makes logging nicer)
//  - deliveryCh:  channel from which to retrieve synced data packets (merged from all concurrent peers)
//  - deliver:     processing callback to deliver data packets into type specific sync schs (usually within `sch`)
//  - wakeCh:      notification channel for waking the fetcher when new tasks are available (or sync completed)
//  - expire:      task callback method to abort requests that took too long and return the faulty peers (traffic shaping)
//  - pending:     task callback for the number of requests still needing sync (detect completion/non-completability)
//  - inFlight:    task callback for the number of in-progress requests (wait for all active syncs to finish)
//  - throttle:    task callback to check if the processing sch is full and activate throttling (bound memory use)
//  - reserve:     task callback to reserve new sync tasks to a particular peer (also signals partial completions)
//  - fetchHook:   tester callback to notify of new tasks being initiated (allows testing the scheduling logic)
//  - fetch:       network callback to actually send a particular sync request to a physical remote peer
//  - cancel:      task callback to abort an in-flight sync request and allow rescheduling it (in case of lost peer)
//  - capacity:    network callback to retrieve the estimated type-specific bandwidth capacity of a peer (traffic shaping)
//  - idle:        network callback to retrieve the currently (type specific) idle peers that can be assigned tasks
//  - setIdle:     network callback to set a peer back to idle and update its estimated capacity (traffic shaping)
//  - kind:        textual label of the type being synced to display in log mesages
func (this *fullSync) fetchParts(errCancel error, deliveryCh chan dataPack, deliver func(dataPack) (int, error), wakeCh chan bool,
		...
		case <-update:
			...
			for pid, fails := range expire() {
				if peer := this.syncer.peers.Peer(pid); peer != nil {
					...
					//
					// The reason the minimum threshold is 2 is because the syncer tries to estimate the bandwidth
			...
			// Send a sync request to all idle peers, until throttled
			...
			for _, peer := range idles {
				...
				if err := fetch(peer, request); err != nil {
					...
					// case, the internal state of the syncer and the sch is very wrong so
...
func (this *fullSync) processHeaders(origin uint64, td *big.Int) error {
	...
	for {
		...
		case headers := <-this.headerProcCh:
			...
			// Signal the content syncers of the availablility of new tasks
			for _, ch := range []chan bool{this.bodyWakeCh, this.receiptWakeCh} {
				select {
				case ch <- true:
				default:
...
func (this *fullSync) importBlockResults(results []*fetchResult) error {
	for len(results) != 0 {
		...
		log.Debug("Inserting synced chain", "items", len(results),
