...
func (b *testBackend) EventMux() *sub.TypeMux {
	return b.mux
}
...
func (b *testBackend) HeaderByNumber(ctx context.Context, blockNr rpc.BlockNumber) (*types.Header, error) {
	...
	if blockNr == rpc.LatestBlockNumber {
		hash = bc.GetHeadBlockHash(b.db)
		num = bc.GetBlockNumber(b.db, hash)
	} else {
		num = uint64(blockNr)
		hash = bc.GetCanonicalHash(b.db, num)
	}
	return bc.GetHeader(b.db, hash, num), nil
}

func (b *testBackend) GetReceipts(ctx context.Context, blockHash common.Hash) (types.Receipts, error) {
	num := bc.GetBlockNumber(b.db, blockHash)
...
func (b *testBackend) SubscribeLogsEvent(ch chan<- []*types.Log) sub.Subscription {
	return b.logsFeed.Subscribe(ch)
}

func (b *testBackend) SubscribeChainEvent(ch chan<- bc.ChainEvent) sub.Subscription {
...
func TestBlockSubscription(t *testing.T) {
	...
	var (
		...
		genesis     = new(bc.Genesis).MustCommit(db)
		chain, _    = bc.GenerateChain(&config.ChainConfig{}, genesis, db, 10, func(i int, gen *bc.BlockGen) {})
		chainEvents = []bc.ChainEvent{}
	)

	for _, blk := range chain {
		chainEvents = append(chainEvents, bc.ChainEvent{Hash: blk.Hash(), Block: blk})
	}
...
func TestLogFilter(t *testing.T) {
	...
	var (
		...
		// posted twice, once as vm.Logs and once as bc.PendingLogsEvent
		allLogs = []*types.Log{
			{Address: firstAddr},
	...
	if err := mux.Post(bc.PendingLogsEvent{Logs: allLogs}); err != nil {
		t.Fatal(err)
	}
...
func TestPendingLogsSubscription(t *testing.T) {
	...
	var (
		...
		convertLogs = func(pl []bc.PendingLogsEvent) []*types.Log {
			var logs []*types.Log
			for _, l := range pl {
	...
	// raise events
	time.Sleep(1 * time.Second)
	// allLogs are type of bc.PendingLogsEvent
