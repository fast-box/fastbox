...
func (bc *BlockChain) InsertHeaderChain(chain []*types.Header, checkFreq int, mode config.SyncMode) (int, error) {
	start := time.Now()
	if i, err := bc.hc.ValidateHeaderChain(chain, checkFreq, mode); err != nil {
		return i, err
	}
